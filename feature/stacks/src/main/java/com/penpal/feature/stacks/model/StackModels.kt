package com.penpal.feature.stacks.model

import android.net.Uri

/**
 * Block model for the notebook editor. Each block represents a content unit that can be rendered,
...
sealed class Block {
    ...
    data class LatexBlock(override val id: String, val expression: String = "") : Block()
...
sealed class StackEvent {
    ...
    data class AddProcessBlock(val mediaType: MediaType, val afterBlockId: String? = null) :
            StackEvent()
    data class UpdateProcessBlockStatus(
            val blockId: String,
