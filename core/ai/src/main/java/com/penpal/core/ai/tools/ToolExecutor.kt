...
class ToolExecutor(
    ...
    suspend fun executeWithTools(
        ...
        while (hasMoreToolCalls && iteration < maxIterations) {
            iteration++
            android.util.Log.d("ToolExecutor", "=== Tool Execution Iteration $iteration ===")
            ...
            inferenceFlow(currentPrompt)
                .onCompletion { cause ->
                    if (cause != null) {
                        android.util.Log.e("ToolExecutor", "Inference flow completed with error: ${cause.message}")
                    }
                }
                .collect { chunk ->
            ...
            if (toolCallsFound.isEmpty()) {
                ...
                android.util.Log.d("ToolExecutor", "No more tool calls found, finishing")
            } else {
                android.util.Log.d("ToolExecutor", "Found ${toolCallsFound.size} tool call(s)")
        ...
        if (iteration >= maxIterations) {
            android.util.Log.w("ToolExecutor", "Reached max iterations ($maxIterations)")
        }
    }
